...
import { openai } from "inngest"; // Removed anthropic import
import { z } from "zod"; // Removed ZodCatch
...
const parsePdfTool = createTool({
    ...
    handler: async ({ pdfUrl }, { step }) => {
        if (!step) throw new Error("Step context is missing");
            ...
            return await step.ai.infer("parse-pdf", {
                ...
                body: {
                    messages: [
                        {
                            role: "user",
                            content: `Please download and read the PDF at this URL: ${pdfUrl}.
